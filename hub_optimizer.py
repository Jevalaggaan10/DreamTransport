...
def build_graph(
    route_stops, route_id_col="route_id", stop_id_col="stop_id", seq_col=None
):
    G = defaultdict(set)
...
def compute_all_metrics(stops, G, route_sequences, hubs, demand_key="daily_passengers"):
    ...
    return {
        ...
        "MaxHops": max_h,
        "Directness": direct,
        "AvgComplexity": avg_complex,
        "stop_to_hub": stop_to_hub,
    }


# ----------------------- Candidate generation -----------------------
def gen_candidates_split(route_sequences, min_split_len=4):
    candidates = []
    for rid, seq in route_sequences.items():
        if len(seq) >= min_split_len:
            # ensure both parts keep at least 3 stops to remain viable
            for cut in range(3, len(seq) - 3):
                seq1 = seq[: cut + 1]
                seq2 = seq[cut:]
...
def gen_candidates_attach_hub(route_sequences, G, hubs, max_insert=6):
    candidates = []
    for rid, seq in route_sequences.items():
        if not seq:
            continue
        ...
        for h in hubs:
            ...
            if path and 1 < len(path) <= max_insert:
                ...
                if new_seq != seq:
                    candidates.append(
                        {"type": "attach_hub", "orig_route": rid, "new_seqs": [new_seq]}
                    )
    return candidates
...
def evaluate_candidate(
    ...
    cand_metrics = compute_all_metrics(stops, G, cand_routes, hubs)
    weights = weights or {
        "PWAvgHops": 0.4,
        "AvgRouteLen": 0.25,
        "HubLoadCV": 0.15,
    ...
    change_magnitude = (
        change_alpha * avg_route_change_cost + (1 - change_alpha) * fraction_changed
    )
    ...
    return {
        ...
        "deltas": deltas,
        "avg_route_change_cost": avg_route_change_cost,
        "fraction_changed": fraction_changed,
        "change_magnitude": change_magnitude,
        "required_percent": required,
        "accepted": accepted,
        "cand_metrics": cand_metrics,
        "routes_changed_count": changed_count,
    }
...
def run_optimizer(
    ...
    topN=3,
    max_iters=50,
    outputs_dir="./hub_optimizer_outputs",
    base_threshold=3,
    alpha_percent=20,
    change_alpha=0.5,
    weights=None,
    demand_key="daily_passengers",
    enforce_route_count_leq_original=False,
    min_increment_percent=2.0,
):
    ...
    baseline_metrics = compute_all_metrics(
        stops, G, route_sequences, hubs, demand_key=demand_key
    )
    weights = weights or {
        "PWAvgHops": 0.4,
        "AvgRouteLen": 0.25,
    ...
    for iteration in range(max_iters):
        ...
        for cand in cands:
            ...
            if (
                enforce_route_count_leq_original
                and len(cand_routes) > original_route_count
            ):
                continue
        ...
        # Apply stricter acceptance: also require minimum incremental improvement
        accepted = [
            (c, e, r)
            for (c, e, r) in evaluated
            if e["accepted"] and e["improvement_percent"] >= min_increment_percent
        ]
        # Fallback: if none passed threshold, pick best positively improving candidates
        if not accepted:
            positives = [
                (c, e, r)
                for (c, e, r) in evaluated
                if e["improvement_percent"] >= min_increment_percent
            ]
            # Prefer candidates with high improvement per route changed, then absolute improvement, then lower change magnitude
            positives_sorted = sorted(
                positives,
                key=lambda x: (
                    x[1]["improvement_percent"] / max(1, x[1]["routes_changed_count"]),
                    x[1]["improvement_percent"],
                    -x[1]["change_magnitude"],
                ),
                reverse=True,
            )
            if positives_sorted:
                accepted = positives_sorted[: max(1, topN if mode == "batch" else 1)]
            else:
                print(
                    "No accepted candidates this iteration and no positive improvements; stopping."
                )
                break
        ...
        if mode == "batch":
            ...
            print(
                f"Applying best candidate: type={best_c['type']}, improvement={best_eval['improvement_percent']:.2f}%, change_mag={best_eval['change_magnitude']:.3f}"
            )
